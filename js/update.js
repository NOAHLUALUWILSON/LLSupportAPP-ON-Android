...
// check update
var checkUrl="https://www.llsupport.cn/llsapp/update/update.html";
function checkUpdate(){
	plus.nativeUI.showWaiting("看看有没有Noah的爱");
...
// download wgt
var wgtUrl="https://www.llsupport.cn/llsapp/update/wget/H57161A18.wgt";
function downWgt(){
	plus.nativeUI.showWaiting("远程接收Noah中");
	plus.downloader.createDownload( wgtUrl, {filename:"_doc/update/"}, function(d,status){
		if ( status == 200 ) { 
			console.log("下载Noah成功："+d.filename);
...
function installWgt(path){
	...
	plus.runtime.install(path,{},function(){
		...
		plus.nativeUI.closeWaiting();
		console.log("安装Noah失败["+e.code+"]："+e.message);
		plus.nativeUI.alert("安装Noah失败["+e.code+"]："+e.message);
	});
}
